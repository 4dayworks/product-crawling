...
import { getHolyZoneId, getStoreList, setGraph, setLastMonthLowPrice, setStoreList } from "./function/product";
...
export const updateByProductId = async ({ page = 0, size = 100000, product_id_list }: updateByProductIdType) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    l(`START [${i + 1}/${max}] product_id: ${String(product.product_id).padStart(6, " ")}`, "blue", "");
    const storeList = await getStoreList(product);
    const result = await setStoreList(product, storeList);

    if (!result) {
      l(`setStoreList result: false [${i}/${max}] product_id: ${String(product.product_id).padStart(6, " ")}`, "red");
      return;
    }
    ...
    l("timestamp", "blue", `${String(product.product_id).padStart(6, " ")} ` + new Date().toISOString());
  }
