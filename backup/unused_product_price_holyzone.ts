...
const updateHolyzone = async (product_id_list?: number[]) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const data2: { user_id: number; low_price: number }[] = await axios
      .post(`${NODE_API_URL}/crawling/product/holyzone/notification`, {
      ...
      .catch((e) =>
        l(
          "Noti Err",
          "red",
          "성지존 알림 오류 /crawling/product/holyzone/notification" + e.code
        )
      );
        ...
        await axios
          .get(
            `${NODE_API_URL}/user/firebase/send/low_price?user_list=${userList}&title=야기야기&message=내가 관심을 보인 ${
              ...
              data2[0].low_price
            )}원으로 내려갔어요⬇️&link=/product/${product.product_id}`
          )
          .catch((e) =>
            l(
              "Noti Err",
              "red",
              "성지존 알림 오류 /user/firebase/send/low_price " + e.code
            )
          );
      }
    ...
    l(
      "timestamp",
      "cyan",
      "product_id:" + product.product_id + ", " + new Date().toISOString()
    );
  }
