...
const exceptCategory = (category: string) => {
  ...
  if (categoryData[1] !== "건강식품") {
    if (categoryData[1] === "다이어트식품" && categoryData[2] === "가르시니아") return true;
    return false;
  }
  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      const manualData:
        ...
        | undefined = await axios(`https://node2.yagiyagi.kr/product/price/manual?product_id=${product_id}`).then(
        (d) => d.data.data
      );
      ...
      } | null = await axios(`https://node2.yagiyagi.kr/product/keyword/id?product_id=${product_id}`).then(
        (d) => d.data.data
      );
      ...
      const productListResult: ItemscoutType[] = await axios(
        ...
        return (d.data.data.productListResult as any[]).filter(
          (p: ItemscoutType) =>
            p.isAd === false && p.isOversea === false && !isExceptionKeyword(p.title) && exceptCategory(p.category)
        );
      });
...
export const updateByItemscout = async (size: number, page: number, is_expert_review: boolean) => {
  ...
  const d: {
    ...
    `https://node2.yagiyagi.kr/product/keyword?size=${size}&page=${page}&is_expert_review=${is_expert_review ? 1 : 0}`
  ).then((d) => d.data.data);
