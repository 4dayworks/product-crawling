  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      const manualData:
        ...
        | undefined = await axios(`https://node2.yagiyai.kr/product/price/manual?product_id=${product_id}`).then(
        (d) => d.data.data
      );
      ...
      } | null = await axios(`https://node2.yagiyai.kr/product/keyword/id?product_id=${product_id}`).then(
        (d) => d.data.data
      );
      ...
      if (originData === null) {
        ...
        await axios.post(`https://node2.yagiyai.kr/product/keyword/id`, {
          keyword: product_name,
          keyword_id: itemscout_keyword_id,
        ...
        await axios.patch(`https://node2.yagiyai.kr/product/keyword/id`, {
          keyword: originData.keyword,
          keyword_id: itemscout_keyword_id,
          yagi_product_id: product_id,
        });
      ...
      if (storeList && storeList.length > 0)
        await axios.post(`https://node2.yagiyai.kr/v2/product/keyword/data`, { data: storeList, keyword_id });
      else {
      ...
      //     is_oversea: p.isOversea === false ? 0 : p.isOversea === true ? 1 : null,
      //   };
      //   await axios.post(`https://node2.yagiyai.kr/product/keyword/data`, data);
      // }
      //#endregion
      ...
      axios
        .post("https://node2.yagiyai.kr/product/price", data)
        .then(() => resolve(true))
        .catch(() => resolve(true));
      //#endregion
    } catch (error) {
...
export const updateByItemscout = async (size: number, page: number, is_expert_review: boolean) => {
  ...
  const d: {
    ...
    `https://node2.yagiyai.kr/product/keyword?size=${size}&page=${page}&is_expert_review=${is_expert_review ? 1 : 0}`
  ).then((d) => d.data.data);
